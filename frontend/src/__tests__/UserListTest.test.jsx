  ...
  describe("Error Handling", () => {
    ...
    test("displays HTTP error when response is not ok", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText("HTTP error! status: 404")
          ).toBeInTheDocument();
        },
        { timeout: 3000 }
      );
    ...
    test("sorts users correctly when dropdown value changes", async () => {
      ...
      await waitFor(
        () => {
          ...
          expect(sortedUserCards[0]).toHaveAttribute(
            "data-testid",
            "user-card-1"
          ); // Charlie first
          expect(sortedUserCards[1]).toHaveAttribute(
            "data-testid",
            "user-card-3"
          ); // Bob second
          expect(sortedUserCards[2]).toHaveAttribute(
            "data-testid",
            "user-card-2"
          ); // Alice third
        },
